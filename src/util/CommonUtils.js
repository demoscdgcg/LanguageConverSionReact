...
import useTimeZoneList from "../constants/TimeZone";
...
const CommonUtil = {
  ...
  isValidNumber(value, minLength, maxLength) {
  ...
  isValidPasitiveNumeric(name) {
    if (name === undefined || name === "") return true;
    let regex = /^(?:[1-9][0-9]{0,3}|[1-4][0-9]{3}|5000)$/;
  ...
  isProducAlphaNumericChAndThSpace(data) {
    if (data === undefined || data === "") return true;
    let regex =
      /^[A-Za-z0-9_\s\u4e00-\u9fa5\u0E00-\u0E7F!@#$%^&*()\-+=\[\]{};:'",.<>?/\\|`~]*$/;
    return regex.test(data);
  ...
  getTimeZoneNameByValue(timeZoneList, timeZone) {
    return timeZoneList?.find((t) => t.value === timeZone)?.name;
  },
  getTimeZoneByValue(timeZoneList, timeZone) {
